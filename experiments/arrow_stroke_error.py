...
def draw_grid_fullscreen(img, step=80):
    h, w = img.shape[:2]
    for x in range(0, w, step): cv2.line(img, (x,0), (x,h), (80,80,80), 1)
    for y in range(0, h, step): cv2.line(img, (0,y), (w,y), (80,80,80), 1)
    cv2.line(img, (w//2,0), (w//2,h), (180,180,180), 1)
    cv2.line(img, (0,h//2), (w,h//2), (180,180,180), 1)
...
def rotate_pts_90cw_1024(pts):
    """
    Rotate MakeMeAHanzi coordinates 90° clockwise.
    Assumes ~1024 coordinate system.
    """
    pts = np.asarray(pts, dtype=np.float32)
    x = pts[:, 0]
    y = pts[:, 1]
...
try:
    ...
    template_medians = [
    rotate_pts_90cw_1024(stroke)
    for stroke in template_medians
    ]
...
with vision.HandLandmarker.create_from_options(options) as hand_landmarker:
    while True:
        ...
        draw_grid_fullscreen(combined, step=80)
